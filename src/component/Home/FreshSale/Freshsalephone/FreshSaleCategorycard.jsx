...
const FreshSaleCategorycard = ({product}) => {
  ...
  return (
    <>
                    <Flex as={Link} to={`/ditals-product/${product._id}/${product?.productname}`} gap={2} bg={'white'}px={2} py={2}>
      {          product?.productimage && product?.productimage.map((image, index)=>{
                                return(
                                    index === 0 && (
                                        <Box position={'relative'} borderRadius={4} overflow={'hidden'} key={index} w={28}h={24} pl={1} >
                ...
                <Flex>
                    <VStack alignItems={'start'}>
                        <Box>
                            <Text fontSize={14}>{product?.productname?.length > 45 ? product.productname.slice(0,45)+'...' : product.productname}</Text>
                        </Box>
